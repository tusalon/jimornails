...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'ea5f3104-5eb9-46f9-8e41-f28e58f9a2c6'; // ID de Jimor Nails 
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Jimor Nails ';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '53721186';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'sandraneydi84@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Jimor Nails !';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'jimornails';
};
...
console.log('✅ config-negocio.js listo para Jimor Nails ');
